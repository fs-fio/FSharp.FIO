                    ...
                    testAllRuntimes "and! x3 - executes in parallel" (fun runtime ->
                        ...
                        let effect: FIO<int * int * int, string> =
                            fio {
                                let! a =
                                    FIO.suspend (fun () ->
                                        (fromSecond.Read().Timeout (TimeSpan.FromSeconds 3.0) (fun ex -> ex.Message))
                                            .FlatMap(fun second ->
                                                sawSecond.Value <- Option.isSome second
                                                (fromThird.Read().Timeout (TimeSpan.FromSeconds 3.0) (fun ex -> ex.Message))
                                                    .Map(fun third ->
                                                        sawThird.Value <- Option.isSome third
                                                        1)))

                                and! b =
                                    FIO.suspend (fun () -> (fromSecond.Write ()).Map(fun _ -> 2))

                                and! c =
                                    FIO.suspend (fun () -> (fromThird.Write ()).Map(fun _ -> 3))
